# -*- coding: utf-8 -*-
"""
Created on Fri May 13 17:27:48 2016
...
class asteriod(three_body):
     def calculate(self):
        ...
        for i in range(1,self.N):
            ...
            ## energy ---
            self.e_o.append(0.5*M_e*(self.vxe[i]**2+self.vye[i]**2)-4*np.pi**2*M_e/r_e - 4*np.pi**2*(M_j/M_s)*M_e/r_ej)
     ...
     def plot(self):
         ...
         plt.xlabel('x(AU)')
         plt.ylabel('y(AU)')
         plt.plot(self.xe,self.ye,lw = 1, label = 'Asteroid number 2')
         #plt.plot(self.xj,self.yj,label = 'Jupiter')
         r = 0.1
         theta = np.linspace(0,2*np.pi,36)
         x = r*np.cos(theta)
     ...
     def energy_dispose(self):
         ...
         plt.xlabel('Time(yr)')
         plt.ylabel('Energy')
         plt.plot(time_record,energy_record, lw = 1,label = 'Orbital energy')
         plt.legend()
         plt.savefig('chapter4_4.18_energy_d276.png', dpi = 256)
         plt.show()'''
         x=np.array(energy_record)
         n = len(energy_record)
         y=np.fft.fft(x)
         y=np.array(abs(y))
         '''
