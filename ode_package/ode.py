...
class ode:
    ...
    def euler(self):                   ###simple Euler method
        ...
        for l in range(self.number):    ###
            y.append([])           ####    setting the initial valye
            y[l].append(self.Y[l]) ###
        for i in range(1,self.N):
            for j in range(self.number):
                for k in range(self.number):
                    self.dep_name[k + 1] = self.dep_name_record[k + 1] + '=' + str(y[k][i - 1])
                self.dep_name[0] = self.dep_name_record[0] + '=' + str(x[i - 1])
                for m in range(self.number + 1):
                    exec self.dep_name[m]
                y[j].append(y[j][i - 1] + eval(self.f[j]) * self.dx)
            x.append(x[i - 1] + self.dx)
        self.data[0] = x
        self.data[1] = y
        return self.data
        ...
        for i in range(1,self.N):
            for j in range(self.number):
                ...
                for m in range(self.number + 1):
                    exec self.dep_name[m]
                v1 = eval(self.f)
                self.dep_name[j + 1] = self.dep_name_record[j + 1] + '=' + str(y[j][i - 1] + v1*self.dx)
                exec self.dep_name[j + 1]
                v2 = eval(self.f)
                y[j].append(y[j][i - 1] + self.dx * (v1 + v2)/2)
            x.append(x[i - 1] + self.dx)
        self.data[0] = x
        self.data[1] = y
        return self.data
        ...
        for i in range(1,self.N):
            for j in range(self.number):
                ...
                for m in range(self.number + 1):
                    exec self.dep_name[m]
                v1 = eval(self.f)
                self.dep_name[j + 1] = self.dep_name_record[j + 1] + '=' + str(y[j][i - 1] + v1*self.dx/2)
                self.dep_name[0] = self.dep_name_record[0] + '=' + str(x[i - 1] + self.dx/2)
                exec self.dep_name[0]
                exec self.dep_name[j + 1]
                v2 = eval(self.f)
                y[j].append(y[j][i - 1] + self.dx * v2)
            x.append(x[i - 1] + self.dx)
        self.data[0] = x
        self.data[1] = y
        return self.data
        ...
        for i in range(1,self.N):
            for j in range(self.number):
                for d in range(self.number):
                    self.dep_name[d + 1] = self.dep_name_record[d + 1] + '=' + str(y[d][i - 1])
                self.dep_name[0] = self.dep_name_record[0] + '=' + str(x[i - 1])
                for m in range(self.number + 1):
                    exec self.dep_name[m]
                k1 = eval(self.f[j])
                self.dep_name[j + 1] = self.dep_name_record[j + 1] + '=' + str(y[j][i - 1] + k1*self.dx/2)
                self.dep_name[0] = self.dep_name_record[0] + '=' + str(x[i - 1]+self.dx/2)
                exec self.dep_name[0]
                exec self.dep_name[j + 1]
                k2 = eval(self.f[j])
                self.dep_name[j + 1] = self.dep_name_record[j + 1] + '=' + str(y[j][i - 1] - k1*self.dx + 2*k2*self.dx)
                self.dep_name[0] = self.dep_name_record[0] + '=' + str(x[i - 1] + self.dx)
                exec self.dep_name[0]
                exec self.dep_name[j + 1]
                k3 = eval(self.f[j])
                y[j].append(y[j][i - 1] + self.dx * (k1 + 4*k2 +k3)/6)
            x.append(x[i - 1] + self.dx)
        self.data[0] = x
        self.data[1] = y
        return self.data 
